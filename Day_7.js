const rules = `clear purple bags contain 5 faded indigo bags, 3 muted purple bags.
...
mirrored maroon bags contain 3 drab coral bags.`.split(".\n");

/**
 * Puzzle 1 - Find the amount of bags that could possibly contain one that is "shiny gold"
 *
 * Find the the bag that contains a the "shiny gold" bag, then the bag that contains it, then the bag that contains IT, etc. until we can't find anymore
 * As we find the containing bags, keep track of them.  Finally, eliminate duplicate bags
 *
 * @param {String} bag name of the bag to search for
 * @returns {Set} bags that could contain a "shiny gold" bag
 */
function digForGold(bag) {
  let bagsFound = [];
  for (rule of rules) {
    const parse = rule.split("contain");
    if (RegExp(bag).test(parse[1])) {
      const newBag = parse[0].replace(/( bag | bags )/, "").trim();
      bagsFound.push(newBag);
      bagsFound = [...bagsFound, ...digForGold(newBag)];
    }
  }
  return bagsFound;
}

console.log(new Set(digForGold("shiny gold")).size);
