// TODO: refactor Puzzle 1 to use the same parsed input format as puzzle 2

const input = `jmqnkzlsfedaptx
usjfkadqwmeyilph
...
const sampleInput = `jmqnkzlsfedaptx
usjfkadqwmeyilph
...
/**
 * Puzzle 1 - Help groups on the plane answer the questions on the customs form
 *
 * @param {Array} inputs the answers to the form separated by group
 * @returns {Number} sum total of all "Yes" answers in the map
 */
function findSumOfYesses(inputs) {
  const yesMap = new Map();
  inputs.forEach((group) => {
    for (let line of group) {
      yesMap.set(line, yesMap.has(line) ? yesMap.get(line) + 1 : 1);
    }
  });
  return countMapOccurences(yesMap);
}

/**
 * Puzzle 2 - the criteria for answering has to be more strict.  In order for qualification,
 * every person in the group must answer YES to the question
 * (in order for 'a' to be 'yes', every person in the group must answer 'yes' to question 'a')
 *
 * @param {Array} inputs the answers to the form separated by group
 * @returns {Number} sum of yesses that were common within groups
 */
function findSumOfCommonYesses(inputs) {
  const allYesses = new Map();
  sampleInput.forEach((group) => {
    const groupSize = group.length;
    const yesMap = new Map();
    for (let line of group) {
      for (let answer of line) {
        yesMap.set(answer, yesMap.has(answer) ? yesMap.get(answer) + 1 : 1);
      }
    }
    for (let i of yesMap.entries()) {
      if (i[1] === groupSize) {
        allYesses.set(i[0], allYesses.has(i[0]) ? allYesses.get(i[0]) + 1 : 1);
      }
    }
  });
  return countMapOccurences(allYesses);
}

/**
 * Helper Function to count the frequency of letter occurences in the map
 *
 * @param {Map} map
 * @returns {Number} sum total of all "Yes" answers in the map
 */
function countMapOccurences(map) {
  let sumYes = 0;
  for (let question of map.values()) {
    sumYes += question;
  }
  return sumYes;
}
...
console.log(
  "Puzzle 1 - total number of unique 'Yes' answers: ",
  findSumOfYesses(input)
);
console.log(
  "Puzzle 2 - total number of full group 'Yes' answers: ",
  findSumOfCommonYesses(sampleInput)
);
