...
2-7 b: fdbvbdb`
  ...
  .map((line) => {
    ...
    return {
      min: params[0].split("-")[0],
      max: params[0].split("-")[1],
      char: params[1][0],
      password: params[2]
    };
  });
 ...
 * A password is valid if a specified character occurs within the given range
 *
 * @param list Array of strings in format: "min-max char: password"
 * @returns Number of valid passwords in the list
 */
function countValidPasswords(list) {
  let count = 0;
  list.forEach((pass) => {
    const { char, min, max, password } = pass;
    const reg = new RegExp(char, "g");
    const matches = password.match(reg);
    if (!matches) {
    } else if (matches.length >= min && matches.length <= max) count++;
  });
  return count;
}
